...
class TestNotLoggedIn(TestCase):
    ...
    def test_gamekeeper_addBin(self):
        ...
        self.assertEqual(response.redirect_chain, [('/gamekeeper/', 302), ('/account/login/', 302)])
...
class TestLoggedIn(TestCase):
    ...
    def test_gamekeeper_addBin_logged(self):
        response = self.client.get('/gamekeeper/addBin', follow=True)
        self.assertEqual(response.redirect_chain, [('/gamekeeper/', 302), ('/', 302)])
...
class TestAdmin(TestCase):
    ...
    def setUp(self):
        self.client = Client()
        self.adminuser = User.objects.create_user(username='testUser', password='PassTest')
    ...
    def test_gamekeeper_admin(self):
        ...
        self.assertEqual(response.context['Goals'][0].get_field('goalID'), 1)
        print(response.context['Goals'][0].get_field('goalID'))
        self.assertEqual(response.context['ShopItems'][0].get_field('item_id'), 1)
        self.assertEqual(response.context['Bins'][0].get_field('binId'), 'XFI-LEC')
    ...
    def test_gamekeeper_deleteBin_admin(self):
        response = self.client.get('/gamekeeper/deleteBin', {'bin': 'XFI-LEC'}, follow=True)
        self.assertEqual(response.redirect_chain, [('/gamekeeper/', 302)])

    def test_gamekeeper_deleteGoal_admin(self):
        response = self.client.get('/gamekeeper/deleteGoal', {'goal': '1'}, follow=True)
        self.assertEqual(response.redirect_chain, [('/gamekeeper/', 302)])

    def test_gamekeeper_deleteShopItem_admin(self):
        response = self.client.get('/gamekeeper/deleteShopItem', {'shopItem': '1'}, follow=True)
        self.assertEqual(response.redirect_chain, [('/gamekeeper/', 302)])
