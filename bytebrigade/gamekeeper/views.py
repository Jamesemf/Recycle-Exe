...
def addBin(request):
    ...
    binName = request.POST['binName']
    binLat = request.POST['binLat']
    binLong = request.POST['binLong']
    binPhoto = request.POST['binPhoto']
...
def addShopItem(request):
    ...
    newShopItem = Goal(name=name, cost=cost, description=description)
    newShopItem.save()
    return redirect('gamekeeperPage')
